...
public class Blacklist extends Command {
    ...
    protected void onServer(String paramString, MessageReceivedEvent event) {
        ...
        try {
                ...
                if (event.getGuild().getSelfMember().hasPermission(Permission.KICK_MEMBERS)) {
                    ...
                    if (members.size() > 0) {
                        ...
                        for (Member m : members) {
                            reportchannel.sendMessage("Kicked " + m.getUser().getEffectiveName() + " by setting blacklist").setAllowedMentions(Collections.emptyList()).queue();
                        }
                    }
                }
            }
            ...
            ExceptionUtils.reportException("Failed to add blacklist", e, event.getChannel());
        }
    }
