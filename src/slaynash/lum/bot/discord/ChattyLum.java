...
public class ChattyLum {
    ...
    public static boolean handle(String message, MessageReceivedEvent event) {
        ...
        if (message.startsWith("ffxiv")) {
            System.out.println("ffxiv");
            event.getChannel().sendMessage("Did you know that the critically acclaimed MMORPG Final Fantasy XIV has a free trial, and includes the entirety of A Realm Reborn, Heavensward, AND the nomination-winning Stormblood expansion up to level 70 with no restrictions on playtime?\nSign up, and enjoy Eorzea today! https://secure.square-enix.com/account/app/svc/ffxivregister?lng=en-gb").queue();
            return true;
        }
        ...
        if (message.matches(".*\\b(g(oo|u)d|best|nice|proud|great(|est)|(be)?love(d|ly)?|nerd|s(hm|)exy|hugs?|beautiful|cool|cuti?e(st)?|adorable|amaz(e|ing)|helped|thanks*|p([ea])ts*|dab)\\b.*")) {
        ...
        if (message.matches(".*\\b(bad|shu(t|sh)|smh|hush|stupid|dumb?|baka|gay|fat|evil)\\b.*")) {
    ...
    public static void addNewHelpedRecently(MessageReceivedEvent event) {
        helpedRecently.removeIf(h -> h.userid == event.getAuthor().getIdLong()); //remove all past log from user
        helpedRecently.removeIf(h -> h.time + helpDuration < Instant.now().getEpochSecond());
        helpedRecently.add(new HelpedRecentlyData(event.getAuthor().getIdLong(), event.getChannel().getIdLong()));
