...
public class VFTableOffsetFinder implements AutoCloseable {
    ...
    private record SectionHeader(int virtualSize, int virtualAddress, int rawSize, int rawAddress) { }

    private final String pdbPath;
    private final boolean is64bit;
    private final SectionHeader[] sections;
    private final RandomAccessFile dllReader;
    ...
    private static SectionHeader[] pdbUtilGetHeadersOffset(String pdbPath) throws IOException, InterruptedException {
        ...
        for (int i = 1; i < sectionsRaw.length; i++) {
            ...
            for (String line : lines) {
                ...
                switch (parts[1]) {
                    case "virtual size" -> virtualSize = Integer.parseInt(parts[0], 16);
                    case "virtual address" -> virtualAddress = Integer.parseInt(parts[0], 16);
                    case "size of raw data" -> rawSize = Integer.parseInt(parts[0], 16);
                    case "file pointer to raw data" -> rawAddress = Integer.parseInt(parts[0], 16);
                    default -> { }
                }
            }
